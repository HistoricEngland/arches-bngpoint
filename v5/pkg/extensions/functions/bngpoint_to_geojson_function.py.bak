...
class BNGPointToGeoJSON(BaseFunction):
    ...
    def save_geojson(self,tile,request,is_function_save_method=True):
        ...
        bngValueReturned = ""
        gridSquare = {  "NA":[0,9],"NB":[1,9],"NC":[2,9],"ND":[3,9],"NE":[4,9],"OA":[5,9],"OB":[6,9],
        ...
        if bngValueReturned != None:
            ...
            on the Search map.
            '''
            if self.config[u"geojson_nodegroup"] == str(tile.nodegroup_id):
                tile.data[geojsonNode] = geometryValueJson
            else:
                previously_saved_tiles = Tile.objects.filter(
                    nodegroup_id=self.config[u"geojson_nodegroup"], resourceinstance_id=tile.resourceinstance_id)
                ...
                if len(previously_saved_tiles) > 0:
                    ...
                    new_geojson_tile = Tile().get_blank_tile_from_nodegroup_id(
                        self.config[u"geojson_nodegroup"], resourceid=tile.resourceinstance_id, parenttile=tile.parenttile)
                    new_geojson_tile.data[self.config[u"geojson_node"]
                                          ] = geometryValueJson

                    if self.config[u"geojson_nodegroup"] in new_geojson_tile.data:
